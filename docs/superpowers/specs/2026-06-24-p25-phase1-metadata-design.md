...
2. **符号时钟:分段重同步** —— 每个 FS 锚点独立恢复本帧 864 符号(180ms,一个 LDU),不做跨帧 PLL(样本每帧都有 FS 可锚定;实测相邻 FS 间距 864 符号/180ms 出现 56 次,主导)。
...
72-bit LC → RS(24,12,13)/GF(2⁶) → 144 bit → Hamming(10,6,3) ×24 → 240 编码 bit → 交织散布在 864 符号 LDU 帧中(与 9×IMBE 语音交错)。GF(2⁶) 本原多项式 x⁶+x+1 (0x43);RS 的 fcr/prim 精确值由里程碑①标准向量锁定。
...
改动:
  p25/constants.py    LDU 帧布局常量(864符号、LC 交织表、各段偏移)
  p25/dsp.py          整帧 864 符号恢复 + 去交织抽 LC
...
```
y → find_frame_sync → recover_full_frame(864符号)
...
tests/test_p25_dsp.py          扩展:整帧 864 恢复 + 去交织(合成向量)
...
1. **交织表正确性(最高)** —— 864 符号里 LC 的精确位置。缓解:里程碑②先用无纠错检错在干净帧上验证抽取的 240 bit 过 Hamming/RS syndrome 为 0。
2. **RS over GF(2⁶) 自实现** —— 生成多项式/根/移位易错。缓解:TIA-102 标准向量单测锁定,参考 `okdmr` RS 骨架。
3. **分段重同步漂移** —— 864 符号末端定时误差。缓解:lstsq 用 FS 区定标,末端若超界则该帧 RS 失败自然丢弃。
