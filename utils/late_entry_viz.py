...
Usage (run from the project root; no -m needed):
  python utils/late_entry_viz.py     # -> output/late_entry_viz.png
"""
import os
import sys
...
# Repo root on path so the root-level modules import when run from utils/.
...
plt.tight_layout()
plt.savefig(_OUT_PNG, dpi=90)
n_ok = sum(1 for r in rows if r[2] and r[2].get('cs5_ok'))
print('saved %s  (%d/%d 超帧 CS5 通过)' % (_OUT_PNG, n_ok, len(rows)))
